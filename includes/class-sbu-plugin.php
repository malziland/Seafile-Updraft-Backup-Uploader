...
final class SBU_Plugin {
    ...
    public function __construct() {
        ...
        add_action( 'admin_init', [ $this, 'check_stalled_queue' ] );
        add_action( 'admin_enqueue_scripts', [ $this, 'enqueue_styles' ] );
        add_action( 'wp_dashboard_setup', [ $this, 'register_widget' ] );
        add_action( 'updraftplus_backup_complete', [ $this, 'on_backup_complete' ] );
        add_action( SBU_CRON_HOOK, [ $this, 'cron_process_queue' ] );
        foreach ( [ 'test', 'upload', 'list', 'download', 'download_all', 'delete', 'get_log', 'export_log', 'export_log_anon', 'clear_log', 'upload_status', 'load_libs', 'load_dirs', 'create_dir', 'save_settings', 'reset_settings', 'refresh_nonce', 'abort_upload', 'pause_upload', 'resume_upload', 'kick', 'dismiss_restore_banner' ] as $a ) {
            add_action( 'wp_ajax_sbu_' . $a, [ $this, 'ajax_' . $a ] );
        }
        // External cron endpoint (no login required, key-protected)
        add_action( 'wp_ajax_nopriv_sbu_cron_ping', [ $this, 'ajax_cron_ping' ] );
        add_action( 'wp_ajax_sbu_cron_ping', [ $this, 'ajax_cron_ping' ] );
    }
    ...
    public function get_settings() {
        return wp_parse_args( get_option( SBU_OPT, [] ), [
            'url' => '', 'user' => '', 'pass' => '', 'lib' => '',
            'folder' => '/', 'chunk' => 40, 'download_chunk' => SBU_DOWNLOAD_CHUNK_MB_DEFAULT,
            'retention' => 4,
            'del_local' => 0, 'auto' => 1,
            'email' => get_option( 'admin_email' ), 'notify' => 'error',
            'debug_log' => 0,
        ] );
    }
    ...
    public function enqueue_styles( $hook ) {
        ...
        $s = $this->get_settings();
        wp_localize_script( 'sbu-admin', 'sbuAdmin', [
    ...
    public function render_page() {
        ...
        $cron_url = add_query_arg(
            [ 'action' => 'sbu_cron_ping', 'key' => $this->get_cron_key() ],
    ...
    public function field( $name, $label, $value, $type = 'text', $placeholder = '', $hint = '' ) {
        echo '<div class="ff"><label>' . $label . '</label>';
        echo '<input type="' . $type . '" name="' . SBU_OPT . '[' . $name . ']" value="' . esc_attr( $value ) . '"';
        if ( $placeholder ) echo ' placeholder="' . esc_attr( $placeholder ) . '"';
        echo ' /></div>';
        if ( $hint ) echo '<p class="hint">' . $hint . '</p>';
    }
    ...
    public function register_widget() {
        wp_add_dashboard_widget( 'sbu_w', 'Seafile Backup', [ $this, 'render_widget' ] );
    }
    ...
    public function render_widget() {
        $st = get_option( SBU_STAT, [] );
        $s = $this->get_settings();
        if ( empty( $st ) ) {
            echo '<p style="color:#646970">' . __( 'Noch kein Backup hochgeladen.', 'seafile-updraft-backup-uploader' ) . '</p>';
            echo '<p><a href="' . admin_url( 'options-general.php?page=' . SBU_SLUG ) . '">' . __( 'Einstellungen', 'seafile-updraft-backup-uploader' ) . ' &rarr;</a></p>';
            return;
        }
        $ok = ! empty( $st['success'] );
        ...
        echo '<span style="font-size:28px">' . $ico . '</span><div>';
        ...
        echo '<tr><td>' . __( 'Dateien:', 'seafile-updraft-backup-uploader' ) . '</td><td style="text-align:right"><strong>' . intval( $st['files'] ?? 0 ) . '</strong></td></tr>';
        echo '<tr><td>' . __( 'Größe:', 'seafile-updraft-backup-uploader' ) . '</td><td style="text-align:right"><strong>' . number_format( $st['size_mb'] ?? 0, 1 ) . ' MB</strong></td></tr>';
        if ( ! empty( $st['errors'] ) ) {
            echo '<tr><td>' . __( 'Fehler:', 'seafile-updraft-backup-uploader' ) . '</td><td style="text-align:right"><strong style="color:#d63638">' . intval( $st['errors'] ) . '</strong></td></tr>';
        }
        echo '<tr><td>' . __( 'Target:', 'seafile-updraft-backup-uploader' ) . '</td><td style="text-align:right">' . esc_html( $s['lib'] . $s['folder'] ) . '</td></tr>';
        if ( ! empty( $st['dir'] ) ) {
            echo '<tr><td>' . __( 'Folder:', 'seafile-updraft-backup-uploader' ) . '</td><td style="text-align:right"><code>' . esc_html( $st['dir'] ) . '</code></td></tr>';
        }
        echo '</table>';
        echo '<p style="margin:12px 0 0"><a href="' . admin_url( 'options-general.php?page=' . SBU_SLUG ) . '">' . __( 'Details', 'seafile-updraft-backup-uploader' ) . ' &rarr;</a></p>';
    }
    ...
    private function verify_ajax_request() {
        if ( ! current_user_can( 'manage_options' ) ) wp_send_json_error( __( 'Zugriff verweigert.', 'seafile-updraft-backup-uploader' ) );
        $nonce = isset( $_POST['_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_nonce'] ) ) : '';
        if ( ! wp_verify_nonce( $nonce, SBU_NONCE ) ) wp_send_json_error( __( 'Sicherheitsprüfung fehlgeschlagen. Bitte Seite neu laden.', 'seafile-updraft-backup-uploader' ) );
    }
    ...
    public function ajax_test() {
        ...
        // Debug: track password source
        $dots = str_repeat( "\xe2\x80\xa2", 8 );
        $raw_post = $_POST['sbu_pass'] ?? '';
    ...
    public function ajax_upload() {
        ...
        if ( $existing && in_array( $existing['status'] ?? '', [ 'uploading', 'restoring' ], true ) ) {
    ...
    public function ajax_abort_upload() {
        ...
        if ( $queue && in_array( $queue['status'], [ 'uploading', 'restoring', 'error' ], true ) ) {
    ...
    public function ajax_pause_upload() {
        ...
        wp_cache_delete( SBU_QUEUE, 'options' );
        $queue = get_option( SBU_QUEUE );
        if ( ! is_array( $queue ) || ! in_array( $queue['status'] ?? '', [ 'uploading', 'restoring' ], true ) ) {
    ...
    public function ajax_list() {
        ...
        $raw_pass = ! empty( $_POST['sbu_pass'] ) ? sanitize_text_field( wp_unslash( $_POST['sbu_pass'] ) ) : '';
        ...
        if ( is_wp_error( $t ) ) wp_send_json_error( __( 'Authentifizierung fehlgeschlagen.', 'seafile-updraft-backup-uploader' ) );
        $r = SBU_Seafile_API::find_library( $url, $t, $lib );
        if ( is_wp_error( $r ) ) wp_send_json_error( __( 'Bibliothek nicht gefunden.', 'seafile-updraft-backup-uploader' ) );
        ...
        if ( is_wp_error( $items ) ) wp_send_json_error( __( 'Verzeichnis nicht lesbar.', 'seafile-updraft-backup-uploader' ) );

        $dirs = [];
        foreach ( $items as $it ) {
            if ( ( $it['type'] ?? '' ) === 'dir' ) $dirs[] = $it['name'];
        }
        if ( empty( $dirs ) ) {
        ...
        foreach ( $dirs as $dn ) {
            ...
            $fc = 0; $ts = 0; $types = [];
            $local_ok = 0; $local_stale = 0; $local_total = 0;
            ...
            // Content type badges
            $badges = '';
            $type_labels = [ 'db' => 'DB', 'plugins' => 'Plugins', 'themes' => 'Themes', 'uploads' => 'Uploads', 'others' => __( 'Andere', 'seafile-updraft-backup-uploader' ) ];
            ...
            if ( $vstat ) {
                if ( $vstat['status'] === 'complete' ) {
                    ...
                    if ( ! empty( $vstat['issues'] ) ) $tip .= ' — ' . implode( ', ', $vstat['issues'] );
    ...
    public function ajax_download() {
        $this->verify_ajax_request();
        @set_time_limit( 600 );
        $dir = $this->sanitize_path_segment( $_POST['dir'] ?? '' );
        $fn  = $this->sanitize_path_segment( $_POST['file'] ?? '' );
        if ( ! $dir || ! $fn ) wp_send_json_error( __( 'Ungültige Parameter.', 'seafile-updraft-backup-uploader' ) );

        $s = $this->get_settings(); $pw = SBU_Crypto::decrypt( $s['pass'] );
        $t = SBU_Seafile_API::get_token( $s['url'], $s['user'], $pw );
        if ( is_wp_error( $t ) ) wp_send_json_error( __( 'Authentifizierungsfehler.', 'seafile-updraft-backup-uploader' ) );
        $r = SBU_Seafile_API::find_library( $s['url'], $t, $s['lib'] );
        if ( is_wp_error( $r ) ) wp_send_json_error( __( 'Bibliotheksfehler.', 'seafile-updraft-backup-uploader' ) );

        $path = rtrim( $s['folder'], '/' ) . '/' . $dir . '/' . $fn;
        $ud = $this->get_updraft_dir();
        if ( ! $ud ) wp_send_json_error( __( 'UpdraftPlus-Verzeichnis nicht gefunden.', 'seafile-updraft-backup-uploader' ) );
    ...
    public function ajax_download_all() {
        ...
        $dir = $this->sanitize_path_segment( $_POST['dir'] ?? '' );
        if ( ! $dir ) wp_send_json_error( __( 'Kein Ordner angegeben.', 'seafile-updraft-backup-uploader' ) );
        ...
        $all_hashes = get_option( SBU_HASHES, [] );
    ...
    public function ajax_export_log() {
        ...
        wp_send_json_success( $header . ( $log ?: __( 'Noch keine Aktivität aufgezeichnet.', 'seafile-updraft-backup-uploader' ) ) );
    }
    ...
    public function ajax_dismiss_restore_banner() {
        $this->verify_ajax_request();
        $last = get_option( 'sbu_last_restore_success', [] );
    ...
    public function ajax_upload_status() {
        ...
        wp_cache_delete( SBU_QUEUE, 'options' );
        $queue = get_option( SBU_QUEUE );
        if ( ! $queue || ! in_array( $queue['status'] ?? '', [ 'uploading', 'restoring', 'paused' ], true ) ) {
    ...
    public function ajax_kick() {
        $this->verify_ajax_request();
        $queue = get_option( SBU_QUEUE );
        if ( ! $queue || ! in_array( $queue['status'] ?? '', [ 'uploading', 'restoring' ], true ) ) {
            wp_send_json_success( [ 'kicked' => false, 'reason' => 'no active queue' ] );
        }
        if ( $this->tick_is_gated() ) {
            wp_send_json_success( [ 'kicked' => false, 'reason' => 'backoff' ] );
        }
        if ( ! $this->acquire_queue_lock( $this->queue_lock_ttl() ) ) {
            wp_send_json_success( [ 'kicked' => false, 'reason' => 'already processing' ] );
        }
        try {
     ...
     * URL: /wp-admin/admin-ajax.php?action=sbu_cron_ping&key=SECRET
     *
     * @return void Sends JSON response.
     */
    public function ajax_cron_ping() {
        ...
        // absorb the wait in-process: sleep up to the tick budget, then
        // either run the tick (gate expired) or hand off to a fresh
        // loopback (gate still held). This keeps the queue moving
        // entirely on its own, without external pings.
        if ( $this->tick_is_gated() ) {
            ...
            $remain  = $gate_ts - time();
            if ( $remain > 0 ) {
                $wait = min( $remain, $budget );
                @set_time_limit( $wait + (int) $lim['tick_time'] + 30 );
                sleep( $wait );
            }
            if ( $this->tick_is_gated() ) {
                $this->spawn_next_tick();
                wp_send_json_success( [ 'status' => 'slept', 'next' => 'loopback' ] );
            }
            // Gate expired during sleep — fall through and run a tick.
        }
        if ( ! $this->acquire_queue_lock( $this->queue_lock_ttl() ) ) {
            wp_send_json_success( [ 'status' => 'locked' ] );
        }
        try {
            $this->process_queue_tick();
        } finally {
    ...
    private function compute_queue_timeout( $queue ) {
        ...
        if ( $total <= 0 ) {
            foreach ( $queue['files'] ?? [] as $f ) {
                $total += (int) ( $f['size'] ?? 0 );
            }
        }
        if ( $total <= 0 ) return (int) SBU_QUEUE_TIMEOUT;
    ...
    public static function compute_retry_delay( int $attempt, string $kind = 'transient' ): int {
        ...
        if ( $kind === 'empty' ) {
            $tiers = [ 60, 300, 900, 1800, 3600 ];
        } else {
            $tiers = [ 60, 120, 240, 480, 960, 1920, 3600 ];
        }
    ...
    public static function classify_chunk_error( array $result ) {
        ...
        if ( in_array( $http, [ 400, 404, 416 ], true ) ) {
            return 'client';
        }
    ...
    public static function update_rate_state( array $state, array $outcome, array $ceilings ) {
        ...
        $mode     = in_array( $state['mode'] ?? '', [ 'cruise', 'slow', 'emergency' ], true )
            ? $state['mode'] : 'cruise';
    ...
    public function ajax_load_libs() {
        ...
        $dots = str_repeat( "\xe2\x80\xa2", 8 );
        if ( $pass === $dots ) $pass = SBU_Crypto::decrypt( $s['pass'] );

        if ( ! $url || ! $user || ! $pass ) wp_send_json_error( __( 'Bitte zuerst URL, Benutzername und Passwort ausfüllen.', 'seafile-updraft-backup-uploader' ) );
        ...
        $r = wp_remote_get( $url . '/api2/repos/', [
        ...
        if ( ! is_array( $repos ) ) wp_send_json_error( __( 'Ungültige Antwort.', 'seafile-updraft-backup-uploader' ) );

        $libs = [];
        foreach ( $repos as $rp ) {
            if ( ! empty( $rp['name'] ) ) {
                $libs[] = [ 'name' => $rp['name'], 'id' => $rp['id'], 'size' => $rp['size'] ?? 0 ];
            }
        }
        usort( $libs, function( $a, $b ) { return strcasecmp( $a['name'], $b['name'] ); } );
    ...
    public function ajax_load_dirs() {
        ...
        if ( ! $lib_name ) wp_send_json_error( __( 'Keine Bibliothek angegeben.', 'seafile-updraft-backup-uploader' ) );

        $t = SBU_Seafile_API::get_token( $creds['url'], $creds['user'], $creds['pass'] );
        if ( is_wp_error( $t ) ) wp_send_json_error( __( 'Authentifizierungsfehler.', 'seafile-updraft-backup-uploader' ) );

        $rid = SBU_Seafile_API::find_library( $creds['url'], $t, $lib_name );
        if ( is_wp_error( $rid ) ) wp_send_json_error( $rid->get_error_message() );
    ...
    public function ajax_create_dir() {
        ...
        if ( ! $lib_name || ! $dirname ) wp_send_json_error( __( 'Parameter fehlen.', 'seafile-updraft-backup-uploader' ) );
    ...
    private function create_upload_queue( $nonce = '' ) {
        ...
        $verified[ $ts ] = [
            ...
            'issues'  => [ __( 'Upload läuft…', 'seafile-updraft-backup-uploader' ) ],
    ...
    public function check_stalled_queue() {
        $queue = get_option( SBU_QUEUE );
        if ( ! $queue || ! in_array( $queue['status'] ?? '', [ 'uploading', 'restoring' ], true ) ) return;
        if ( ( time() - ( $queue['last_activity'] ?? 0 ) ) < 90 ) return;
        if ( $this->tick_is_gated() ) return;
    ...
    private function process_queue_tick() {
        ...
        while ( $queue['file_idx'] < count( $queue['files'] ) ) {
            if ( $this->tick_budget_exhausted( $tick_start ) ) break;
     ...
     *
     * @param string $fn       Filename (for log messages).
     ...
     */
    private function verify_restored_file( $fn, $dest, $expected ) {
    ...
    private function process_restore_tick( $queue ) {
        ...
        foreach ( $queue['files'] as $ridx => $rf ) {
            $rstatus = $rf['status'] ?? '';
            if ( $rstatus === 'done' ) $done_count++;
            elseif ( $ridx === ( $queue['file_idx'] ?? 0 ) ) $current_fn = $rf['name'];
            elseif ( $rstatus !== 'error' ) $pending_count++;
        }
        $this->activity_log(
            'TICK',
            sprintf(
        ...
        while ( ( $queue['file_idx'] ?? 0 ) < count( $queue['files'] ) ) {
            if ( $this->tick_budget_exhausted( $tick_start ) ) break;
            ...
            if ( $offset === 0 && $fs > 0 && $fs <= $stream_cap_bytes && ! $stream_tried ) {
                ...
                $stream_res    = SBU_Seafile_API::download_whole_file_stream(
                    $s['url'], $t, $rid, $path, $dest, $stream_budget
                );
            ...
            while ( $offset < $fs ) {
                ...
                for ( $n = 0; $n < $parallel && $cursor < $fs; $n++ ) {
                    ...
                    if ( is_wp_error( $link_n ) ) {
                        ...
                        if ( is_wp_error( $link_n ) ) break 2; // Next tick retries
                    }
                ...
                if ( $has_error && $prefix_ok_count === 0 && ! $deadline_hit_any ) {
                    foreach ( $results as $r ) {
                        if ( ! empty( $r['tmp'] ) && file_exists( $r['tmp'] ) ) @unlink( $r['tmp'] );
                    }
                    $t_retry = SBU_Seafile_API::get_token( $s['url'], $s['user'], $pw, true );
                    if ( ! is_wp_error( $t_retry ) ) {
                        $t = $t_retry;
                        ...
                        foreach ( $ranges as $ri => $range ) {
                            ...
                            if ( is_wp_error( $fresh ) ) { $refresh_ok = false; break; }
                            $ranges[ $ri ]['url'] = $fresh;
                        }
                        if ( $refresh_ok ) {
                            $this->activity_log(
                ...
                if ( $prefix_ok_count > 0 ) {
                    ...
                    $i_written  = 0;
                    foreach ( $results as $r ) {
                        if ( $i_written >= $prefix_ok_count ) break;
                        $th = @fopen( $r['tmp'], 'rb' );
                        if ( $th ) {
                            while ( ! feof( $th ) ) fwrite( $fh, fread( $th, 65536 ) );
                            fclose( $th );
                        }
                        @unlink( $r['tmp'] );
                        if ( ( $r['code'] ?? 0 ) === 200 ) {
                ...
                if ( $partial_prefix ) {
                    $first_err = null;
                    foreach ( $results as $r ) {
                        if ( empty( $r['ok'] ) && isset( $r['error'] ) ) { $first_err = $r['error']; break; }
                    }
        ...
        if ( ( $queue['file_idx'] ?? 0 ) >= count( $queue['files'] ) ) {
            ...
            if ( $err > 0 ) {
                ...
                update_option( 'sbu_last_restore_success', [
                    'ts'        => time(),
                    'dir'       => $dir,
                    'dir_nice'  => $dir_nice,
                    'files'     => (int) $ok,
    ...
    private function upload_one_chunk( $link, $tok, $bdir, $fp, $offset, $fs, $csz ) {
        ...
        $fh = fopen( $fp, 'rb' );
        if ( ! $fh ) return new \WP_Error( 'read', "Cannot open: {$fn}" );
        if ( $offset > 0 ) fseek( $fh, $offset );
        $data = fread( $fh, $len );
        fclose( $fh );

        if ( $data === false ) return new \WP_Error( 'read', "Read error: {$fn}" );
        ...
        $resp = wp_remote_post( $link . '?ret-json=1', [
    ...
    private function validate_response( $resp, $ctx ) {
        if ( is_wp_error( $resp ) ) return new \WP_Error( 'net', "{$ctx}: " . $resp->get_error_message() );
    ...
    private function finish_queue( $queue ) {
        ...
        if ( ! is_wp_error( $t ) ) {
            ...
            if ( $vresult['status'] === 'complete' ) {
                ...
                $issue_list = implode( ', ', $vresult['issues'] ?? [] );
    ...
    private function verify_backup( $url, $t, $rid, $bdir, $queue ) {
        ...
        if ( is_wp_error( $remote_files ) ) {
            return [ 'status' => 'error', 'msg' => 'Verzeichnis nicht lesbar', 'checked' => current_time( 'd.m.Y H:i' ) ];
        }
    ...
    private function enforce_retention( $s, $t, $rid ) {
        if ( intval( $s['retention'] ) === 0 ) return; // 0 = keep all
        ...
        if ( count( $dirs ) <= $s['retention'] ) return;
    ...
    private function cleanup_updraft_history() {
        ...
        foreach ( $history as $ts => $backup ) {
            ...
            $has_local = false;
            $file_keys = [ 'plugins', 'themes', 'uploads', 'others', 'db' ];
    ...
    private function send_notification( $ok, $msg ) {
        $s = $this->get_settings();
        if ( $s['notify'] === 'never' ) return;
        if ( $s['notify'] === 'error' && $ok ) return;
        if ( empty( $s['email'] ) ) return;
