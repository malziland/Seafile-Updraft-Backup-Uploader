...
final class SBU_Seafile_API {
    ...
    public static function get_token( $url, $user, $pass, $force = false ) {
        ...
        $response = wp_remote_post( $url . '/api2/auth-token/', [
    ...
    public static function find_library( $url, $token, $name ) {
        $response = wp_remote_get( $url . '/api2/repos/', [
            'timeout' => SBU_TIMEOUT_API,
    ...
    public static function download_chunks_parallel( $token, $ranges, $chunk_timeout = 0, $deadline_ts = 0.0 ) {
        ...
        foreach ( $ranges as $i => $range ) {
            ...
            $fp  = @fopen( $tmp, 'wb' );
            if ( ! $fp ) {
                $handles[ $i ] = [ 'ch' => null, 'fp' => null, 'tmp' => $tmp, 'start' => (int) $range['start'], 'end' => (int) $range['end'], 'open_fail' => true ];
                continue;
            }
        ...
        foreach ( $handles as $i => $h ) {
            ...
            if ( $h['deadline_hit'] ) {
                @unlink( $h['tmp'] );
                $results[ $i ] = array_merge( $base, [ 'ok' => false, 'error' => new \WP_Error( 'deadline', 'Tick-Deadline erreicht' ), 'tmp' => null ] );
            } elseif ( $err ) {
                @unlink( $h['tmp'] );
                ...
                $results[ $i ] = array_merge( $base, [ 'ok' => false, 'error' => new \WP_Error( 'http', $err ), 'tmp' => null ] );
            } elseif ( $code !== 206 && $code !== 200 ) {
                @unlink( $h['tmp'] );
                $results[ $i ] = array_merge( $base, [ 'ok' => false, 'error' => new \WP_Error( 'http', "HTTP {$code}" ), 'tmp' => null ] );
            } elseif ( $size === 0 ) {
                @unlink( $h['tmp'] );
                $results[ $i ] = array_merge( $base, [ 'ok' => false, 'error' => new \WP_Error( 'empty', '0 bytes received' ), 'tmp' => null ] );
            } else {
                $results[ $i ] = array_merge( $base, [ 'ok' => true, 'tmp' => $h['tmp'], 'bytes' => $size ] );
            }
        }
        curl_multi_close( $mh );
