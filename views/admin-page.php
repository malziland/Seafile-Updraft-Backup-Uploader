 ...
 * @var string                $dots
 * @var string                $activity
 * @var string                $cron_url External cron endpoint URL with embedded key
 */
...
<div class="wrap sbu">
    ...
    $last_restore = get_option( 'sbu_last_restore_success', [] );
    if ( ! empty( $last_restore['ts'] ) && empty( $last_restore['dismissed'] ) ) :
        ...
        <div class="sbu-restore-banner" id="sbu-restore-banner">
            <div class="sbu-restore-banner-head">
                ...
                <button type="button" class="sbu-restore-dismiss" aria-label="<?php esc_attr_e( 'Hinweis schließen', 'seafile-updraft-backup-uploader' ); ?>" onclick="sbuDismissRestoreBanner()">×</button>
            </div>
            <p>
                <?php
                printf(
    ...
    <div class="sbu-grid">
        ...
        <div class="sbu-col-right">
            <div class="card">
                ...
                <div class="acts">
                    <button class="button" onclick="sA('sbu_test',this)"><?php esc_html_e( 'Verbindung testen', 'seafile-updraft-backup-uploader' ); ?></button>
                    <button class="button button-primary" onclick="sA('sbu_upload',this)"><?php esc_html_e( 'Backups jetzt hochladen', 'seafile-updraft-backup-uploader' ); ?></button>
                </div>
            ...
            <div class="sbu-up-banner" id="sbu-upb">
                <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
                    ...
                    <div style="display:flex;gap:6px">
                        <button class="button button-small" id="sbu-pause" onclick="sbuPause()"><?php esc_html_e( 'Pause', 'seafile-updraft-backup-uploader' ); ?></button>
                        <button class="button button-small" id="sbu-resume" style="display:none" onclick="sbuResume()"><?php esc_html_e( 'Fortsetzen', 'seafile-updraft-backup-uploader' ); ?></button>
                        <button class="button button-small" id="sbu-abort" style="color:#b32d2e;border-color:#b32d2e" onclick="sbuAbort()"><?php esc_html_e( 'Abbrechen', 'seafile-updraft-backup-uploader' ); ?></button>
                    </div>
                </div>
            ...
            <details class="card" id="sbu-cronbox" style="padding:10px 16px">
                ...
                <p><?php echo wp_kses_post( __( 'Die folgende URL ist nur für den Ausnahmefall dokumentiert, in dem die Hosting-Umgebung WordPress-Loopbacks komplett blockiert (z. B. Firewall-Regeln, die die Seite sich selbst nicht aufrufen lassen). In dem Fall kann ein beliebiger externer Cron diese URL anpingen — der Regelbetrieb benötigt das nicht.', 'seafile-updraft-backup-uploader' ) ); ?></p>
                <pre style="background:#f6f7f7;padding:10px;border-radius:4px;white-space:pre-wrap;word-break:break-all;font-size:12px;margin:10px 0"><code><?php echo esc_html( $cron_url ); ?></code></pre>
                <p class="hint" style="margin-top:4px"><?php esc_html_e( 'Crontab-Beispiel, alle 2 Minuten:', 'seafile-updraft-backup-uploader' ); ?></p>
